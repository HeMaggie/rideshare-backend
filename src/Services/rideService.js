...
//(Done)2. If a user is already in {user_shared}, then do not decrease seat by 1
...
const fetchAllRides = async () => {
  const rideInfo = {
    ...
    ProjectionExpression:
      "rid, rider, start_city, end_city, time_available, seats_available, price, is_open, user_shared",
  };
...
const fetchOpenRides = async (address) => {
  const rideInfo = {
    ...
    ProjectionExpression:
      "rid, rider, start_city, end_city, time_available, seats_available, price, is_open",
  };
...
const updateSeats = async (rideId, userId) => {
  const seatInfo = {
    ...
    `,
    ConditionExpression: "seats_available > :seatVal AND NOT contains(user_shared, :uid)",
    ExpressionAttributeValues: {
      ":decVal": 1,
  ...
  try {
    ...
    if (error.code === "ConditionalCheckFailedException") {
      console.log("Condition not met, no update performed.");
    } else {
